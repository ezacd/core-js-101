...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const verticalOverlap = top1 < bottom2 && bottom1 > top2;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const intervalString = `${startBracket}${minNumber}, ${maxNumber}${endBracket}`;
...
function isCreditCardNumber(ccn) {
  const ccnArray = String(ccn).split('').reverse().map(Number);

  for (let i = 1; i < ccnArray.length; i += 2) {
    ccnArray[i] *= 2;
    if (ccnArray[i] > 9) {
      ccnArray[i] -= 9;
    }
  ...
  const sum = ccnArray.reduce((acc, digit) => acc + digit, 0);
...
function getDigitalRoot(num) {
  ...
  return String(sum).split('').map(Number).reduce((res, item) => res + item, 0);
}
...
function isBracketsBalanced(str) {
  ...
  for (let char = 0; char < str.length; char += 1) {
    if (brackets[str[char]]) {
      const lastOpeningBracket = stack.pop();
      if (lastOpeningBracket !== brackets[str[char]]) {
        return false;
      }
    } else {
      stack.push(str[char]);
